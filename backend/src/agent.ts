...
const HEARD_LABELS: Record<LanguageCode, string> = {
  ...
  "hi-IN": "🗣️ आपने कहा",
  "bn-IN": "🗣️ আপনি বলেছেন",
  "te-IN": "🗣️ మీరు చెప్పారు",
  "mr-IN": "🗣️ तुम्ही म्हणालात",
  "ta-IN": "🗣️ நீங்கள் சொன்னது",
  "gu-IN": "🗣️ તમે કહ્યું",
  "kn-IN": "🗣️ ನೀವು ಹೇಳಿದ್ದು",
  "ml-IN": "🗣️ നിങ്ങൾ പറഞ്ഞത്",
  "pa-IN": "🗣️ ਤੁਸੀਂ ਕਿਹਾ",
  "or-IN": "🗣️ ଆପଣ କହିଲେ",
};

/**
 * A plain-text echo of a voice note's transcription so the seller can confirm
 * we heard them correctly. Sent as its own message before the actual reply so
 * the spoken reply (TTS) is not polluted with the seller's own words.
 */
...
Capture the product title first. The moment the seller names or describes an item to sell (in text, a voice note, or a photo caption), call update_draft to set a short product title in their own words, and include any price or quantity from that same message in the same update_draft call so every fact attaches to the right product. Never keep asking which product they want to list once they have already described one, and never save a price or quantity for a product without also giving it a title.
...
async function processInboundMessage(message: InboundMessage): Promise<void> {
  ...
  try {
    ...
    // Echo the transcription of any voice note back to the seller first, so
    // they can see exactly what we heard before the actual reply arrives.
    if (message.type === "audio" && turn.content.trim().length > 0) {
      await sender.sendText(message.from, heardEcho(language, turn.content.trim()));
    }
